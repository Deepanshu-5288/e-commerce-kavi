...
class Product {
  ...
  async getProductByRatings(req, res) {
    let { range } = req.body;
    if (!range) {
      return res.json({ error: "All filled must be required" });
    } else {
  ...
  async postAddReview(req, res) {
    ...
    if (!pId || !rating || !review || !uId) {
      ...
      let checkReviewRatingExists = await productModel.findOne({ _id: pId });
          let avg=0;
          checkReviewRatingExists.pRatingsReviews.forEach(rev =>{
          if(rev.user !== uId){
            avg+=rev.rating
          }
          })
          avg+=rating;
          const denom = 1;
          if(checkReviewRatingExists.pRatingsReviews.length > 1){
            denom = checkReviewRatingExists.pRatingsReviews.length
          }
          
          let ratings = avg /denom;
