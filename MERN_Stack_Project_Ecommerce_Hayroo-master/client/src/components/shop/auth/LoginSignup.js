...
import { googleLoginReq } from "./fetchApi";
...
const LoginSignup = (props) => {
  ...
  const loginSignupModalToggle = () =>{
    if(data.loginSignupModal){
      ...
      dispatch({ type: "forgetPasswordModalToggle", payload: false })
    }
    else dispatch({ type: "loginSignupModalToggle", payload: true });
  }
  ...
  const changeLoginSignup = () => {
    ...
    dispatch({ type: "LoginOrSignup", payload: login })
  };
  const logInWithGoogle = async () =>{
    const responseData = await googleLoginReq();
